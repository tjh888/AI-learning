...
class bottleneck(nn.Module):
    ...
    def __init__(self, in_c, out_c, stride=1, downsample=None):
        ...
        self.bn2 = nn.BatchNorm2d(out_c)
        self.conv3 = nn.Conv2d(out_c, out_c * self.expansion, kernel_size=1, stride=1, bias=False)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        layers = []
        layers.append(block(self.in_c, channel, stride, downsample = downsample))
        self.in_c = channel * block.expansion
        for _ in range(1, block_num):
            layers.append(block(self.in_c, channel))
...
def resnet50(num_classes=3, include_top=True):
    return ResNet(bottleneck, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top)

def resnet101(num_classes=3, include_top=True):
    return ResNet(bottleneck, [3, 4, 23, 3], num_classes=num_classes, include_top=include_top)

def resnet152(num_classes=3, include_top=True):
